...
import 'package:jbiaoapp/widgets/mydrawer.dart';
...
class TmListPage extends StatefulWidget {
  ...
  TmListPage({Key key, this.isLeading: true}) : super(key: key);
  @override
...
class TmListPageState extends State<TmListPage> {
  ...
  Widget listItem(BuildContext context, int index) {
    ...
    if (title == Constants.END_LINE_TAG) {
      return new Row(
        ...
        children: <Widget>[Text('---  ${Constants.END_LINE_TAG}  ---')],
      );
    }
          ...
          new Row(
            ...
            children: <Widget>[
              ...
              Container(
                  ...
                  child: new Image.network(listData[index]['TmImg'],
                      width: 120.0, height: 90.0, fit: BoxFit.fill)),
              //右边简介,4行
              Expanded(
                  child: Container(
                      margin: EdgeInsets.all(5.0),
                      child: new Column(
                        ...
                        children: <Widget>[
                          new Row(
                            mainAxisAlignment: MainAxisAlignment.start,
                            children: <Widget>[
                              Text(listData[index]['TmName'],
                                  style:
                                      TextStyle(fontWeight: FontWeight.bold)),
                              Container(
                                  margin: EdgeInsets.only(left: 15.0, top: 3.0),
                                  child: Text('注册号:${listData[index]['RegNo']}',
                                      style: TextStyle(
                          ...
                          new Row(
                            ...
                            children: <Widget>[
                              Container(
                                  ...
                                  child: Text('分类:第${listData[index]['Type']}类',
                          ...
                          new Row(
                            ...
                            children: <Widget>[
                              Container(
                                  width: 250.0,
                                  child: Text(listData[index]['UseRange'],
                                      maxLines: 3,
                          ...
                          new Row(
                            ...
                            children: <Widget>[
                              new Container(
                                  margin:
                                      EdgeInsets.only(top: 2.0, right: 10.0),
                                  height: 20.0,
                                  width: 50.0,
                                  child: RaisedButton(
                                    color: Colors.blue,
  ...
  void routeToDetail(int tId) {
    Navigator.of(context).push(
        new MaterialPageRoute(builder: (ctx) => new DetailPage(tmId: tId)));
  }
  ...
  getTmList() {
    ...
    NetUtils.post(url, postParam).then((data) {
      setState(() {
        List list1 = new List();
        totalCount = json.decode(data)['Data']['Count'];
        if (curPageIndex == 1) {
          listData = null;
          listData = new List();
  ...
  void drawerCallBack(val) {
    ...
    setState(() {
      ...
      if (searchInfo.tmTypes != '' && searchInfo.tmTypes.isNotEmpty) {
        tmType = Util.trimEnd(searchInfo.tmTypes, ',');
      }
      if (searchInfo.cbTypes != '' && searchInfo.cbTypes.isNotEmpty) {
        chType = Util.trimEnd(searchInfo.cbTypes, ',');
      }
      if (searchInfo.tmNameKey != '' && searchInfo.tmNameKey.isNotEmpty) {
        tmNameStr = searchInfo.tmNameKey;
      }
      if (searchInfo.tmRegnoKey != '' && searchInfo.tmRegnoKey.isNotEmpty) {
        regnoStr = searchInfo.tmRegnoKey;
      }
      if (searchInfo.tmRangeKey != '' && searchInfo.tmRangeKey.isNotEmpty) {
        rangeStr = searchInfo.tmRangeKey;
      }
      if (searchInfo.tmGroupKey != '' && searchInfo.tmGroupKey.isNotEmpty) {
        groupStr = searchInfo.tmGroupKey;
      }
      len = searchInfo.len;
      maxPrice = searchInfo.price;
    });
    getTmList();
  }
    ...
    return new Scaffold(
        ...
        body: new DefaultDropdownMenuController(
            ...
            child: new Column(children: <Widget>[
              ...
              new Expanded(
                child: new Stack(
                  children: <Widget>[listItemWidgets(), buildDropdownMenu()],
                ),
              )
            ])));
  }
